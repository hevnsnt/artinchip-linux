"""DNS ad-blocker dashboard for tinyscreen bar display.

Supports AdGuard Home and Pi-hole. Displays query stats, block rates,
top blocked domains, and top clients.

Configuration via environment variables:
  ADGUARD_URL=http://192.168.1.50    (AdGuard Home)
  ADGUARD_USER=admin
  ADGUARD_PASS=password
  -- or --
  PIHOLE_URL=http://192.168.1.50     (Pi-hole)
  PIHOLE_API_KEY=abc123              (from Pi-hole admin → Settings → API)
"""

import os
import time
import threading
from PIL import Image, ImageDraw, ImageFont, ImageFilter

# ── Colors ─────────────────────────────────────────────────────────
BG          = (5, 7, 12)
PANEL_BG    = (10, 14, 24)
ACCENT      = (0, 210, 255)
TEXT        = (220, 225, 240)
TEXT_DIM    = (65, 75, 100)
TEXT_BRIGHT = (252, 254, 255)
GREEN       = (0, 255, 140)
RED         = (255, 50, 50)
YELLOW      = (255, 225, 0)
ORANGE      = (255, 165, 30)
CYAN        = (0, 240, 255)
PURPLE      = (160, 110, 255)

# ── Font cache ─────────────────────────────────────────────────────
_fonts = {}

def font(size):
    if size not in _fonts:
        for path in ['/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf',
                     '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf']:
            try:
                _fonts[size] = ImageFont.truetype(path, size)
                return _fonts[size]
            except Exception:
                continue
        _fonts[size] = ImageFont.load_default()
    return _fonts[size]

# ── Visual utilities ───────────────────────────────────────────────
def _lerp_color(c1, c2, t):
    t = max(0.0, min(1.0, t))
    return tuple(int(a + (b - a) * t) for a, b in zip(c1, c2))

_bg_cache = {}

def _get_bg(w, h):
    if (w, h) not in _bg_cache:
        img = Image.new('RGBA', (w, h), BG + (255,))
        draw = ImageDraw.Draw(img)
        for y in range(h):
            t = y / max(h - 1, 1)
            c = _lerp_color((8, 12, 22), (4, 6, 12), t)
            draw.line([(0, y), (w, y)], fill=c)
        grid_c = (18, 24, 38, 50)
        for gx in range(0, w, 40):
            draw.line([(gx, 0), (gx, h)], fill=grid_c)
        for gy in range(0, h, 40):
            draw.line([(0, gy), (w, gy)], fill=grid_c)
        for gx in range(0, w, 80):
            for gy in range(0, h, 80):
                draw.ellipse([gx - 2, gy - 2, gx + 2, gy + 2], fill=ACCENT + (25,))
                draw.ellipse([gx - 1, gy - 1, gx + 1, gy + 1], fill=ACCENT + (45,))
        for i in range(50):
            a = int(30 * (1.0 - i / 50))
            draw.line([(0, h - 1 - i), (w, h - 1 - i)], fill=(0, 100, 180, a))
        vig = Image.new('RGBA', (w, h), (0, 0, 0, 0))
        vd = ImageDraw.Draw(vig)
        for i in range(40):
            a = int(50 * (1.0 - i / 40))
            vd.line([(0, i), (w, i)], fill=(0, 0, 0, a))
            vd.line([(0, h - 1 - i), (w, h - 1 - i)], fill=(0, 0, 0, a))
        for i in range(60):
            a = int(40 * (1.0 - i / 60))
            vd.line([(i, 0), (i, h)], fill=(0, 0, 0, a))
            vd.line([(w - 1 - i, 0), (w - 1 - i, h)], fill=(0, 0, 0, a))
        img = Image.alpha_composite(img, vig)
        _bg_cache[(w, h)] = img
    return _bg_cache[(w, h)].copy()

_scanline_cache = {}

def _get_scanlines(w, h):
    key = (w, h)
    if key not in _scanline_cache:
        sl = Image.new('RGBA', (w, h), (0, 0, 0, 0))
        sd = ImageDraw.Draw(sl)
        for y in range(0, h, 3):
            sd.line([(0, y), (w, y)], fill=(0, 0, 0, 55))
        _scanline_cache[key] = sl
    return _scanline_cache[key]

def _draw_glow_bar(draw, img, x, y, w, h, pct, color):
    draw.rectangle([x, y, x + w, y + h], fill=(12, 15, 22))
    fill_w = max(0, int(w * min(pct, 100.0) / 100.0))
    if fill_w <= 0:
        return
    pad = 12
    gw, gh = fill_w + pad * 2, h + pad * 2
    if gw < 1 or gh < 1:
        return
    glow = Image.new('RGBA', (gw, gh), (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gd.rectangle([pad, pad, pad + fill_w, pad + h], fill=color + (100,))
    glow = glow.filter(ImageFilter.GaussianBlur(radius=8))
    img.paste(glow, (x - pad, y - pad), glow)
    for col in range(fill_w):
        t = col / max(fill_w - 1, 1)
        c = _lerp_color(tuple(max(0, v - 80) for v in color), color, t * 0.8 + 0.2)
        draw.line([(x + 1 + col, y + 1), (x + 1 + col, y + h - 1)], fill=c)
    tip_w = min(4, fill_w)
    bright = tuple(min(255, c + 60) for c in color)
    draw.rectangle([x + fill_w - tip_w, y, x + fill_w, y + h], fill=bright)

def _draw_hero_text(draw, img, x, y, text, color, size):
    f = font(size)
    bbox = f.getbbox(text)
    tw = bbox[2] - bbox[0] + 20
    th = bbox[3] - bbox[1] + 20
    pad = 8
    glow = Image.new('RGBA', (tw + pad * 2, th + pad * 2), (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gd.text((pad - bbox[0], pad - bbox[1]), text, fill=color + (90,), font=f)
    glow = glow.filter(ImageFilter.GaussianBlur(radius=5))
    img.paste(glow, (x - pad, y - pad), glow)
    draw.text((x, y), text, fill=color, font=f)

def _draw_panel(draw, img, x, y, w, h, accent_color=ACCENT):
    top_c = (14, 18, 30)
    bot_c = (8, 11, 20)
    for row in range(h):
        t = row / max(h - 1, 1)
        c = _lerp_color(top_c, bot_c, t)
        draw.line([(x, y + row), (x + w, y + row)], fill=c)
    glow = Image.new('RGBA', (w, 18), (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gd.rectangle([0, 0, w, 2], fill=accent_color + (220,))
    gd.rectangle([0, 2, w, 6], fill=accent_color + (50,))
    glow = glow.filter(ImageFilter.GaussianBlur(radius=4))
    img.paste(glow, (x, y - 2), glow)

# ── API backends ───────────────────────────────────────────────────

def _fetch_adguard(url, user, passwd):
    """Fetch stats from AdGuard Home API."""
    import requests
    auth = (user, passwd) if user else None
    data = {}
    try:
        # Status
        r = requests.get(f'{url}/control/status', auth=auth, timeout=5)
        r.raise_for_status()
        status = r.json()
        data['running'] = status.get('running', False)
        data['protection'] = status.get('protection_enabled', False)
        data['version'] = status.get('version', '?')

        # Stats
        r = requests.get(f'{url}/control/stats', auth=auth, timeout=5)
        r.raise_for_status()
        stats = r.json()
        data['total_queries'] = stats.get('num_dns_queries', 0)
        data['blocked'] = stats.get('num_blocked_filtering', 0)
        data['blocked_pct'] = (data['blocked'] / data['total_queries'] * 100
                               if data['total_queries'] > 0 else 0)
        data['avg_ms'] = stats.get('avg_processing_time', 0) * 1000
        data['top_blocked'] = stats.get('top_blocked_domains', [])[:8]
        data['top_clients'] = stats.get('top_clients', [])[:6]
        data['queries_over_time'] = stats.get('dns_queries', [])
        data['blocked_over_time'] = stats.get('blocked_filtering', [])
        data['replaced_safebrowsing'] = stats.get('replaced_safebrowsing', 0)
        data['replaced_parental'] = stats.get('replaced_parental', 0)
        data['backend'] = 'AdGuard'
        data['error'] = None
    except Exception as e:
        data['error'] = str(e)[:80]
    return data


def _fetch_pihole(url, api_key):
    """Fetch stats from Pi-hole API."""
    import requests
    data = {}
    try:
        params = {}
        if api_key:
            params['auth'] = api_key
        r = requests.get(f'{url}/admin/api.php?summary', params=params, timeout=5)
        r.raise_for_status()
        summary = r.json()
        data['total_queries'] = int(summary.get('dns_queries_today', 0))
        data['blocked'] = int(summary.get('ads_blocked_today', 0))
        data['blocked_pct'] = float(summary.get('ads_percentage_today', 0))
        data['protection'] = summary.get('status') == 'enabled'
        data['running'] = True
        data['version'] = summary.get('gravity_last_updated', {}).get('relative', {}).get('days', '?')

        # Top items
        r2 = requests.get(f'{url}/admin/api.php?topItems=8', params=params, timeout=5)
        if r2.status_code == 200:
            top = r2.json()
            data['top_blocked'] = [
                {d: c} for d, c in list(top.get('top_ads', {}).items())[:8]
            ]
            data['top_clients'] = [
                {d: c} for d, c in list(top.get('top_sources', {}).items())[:6]
            ]
        else:
            data['top_blocked'] = []
            data['top_clients'] = []

        # Over time data
        r3 = requests.get(f'{url}/admin/api.php?overTimeData10mins', params=params, timeout=5)
        if r3.status_code == 200:
            ot = r3.json()
            data['queries_over_time'] = list(ot.get('domains_over_time', {}).values())[-48:]
            data['blocked_over_time'] = list(ot.get('ads_over_time', {}).values())[-48:]

        data['avg_ms'] = 0
        data['backend'] = 'Pi-hole'
        data['error'] = None
    except Exception as e:
        data['error'] = str(e)[:80]
    return data


# ── State ──────────────────────────────────────────────────────────
_cache = {
    'data': {},
    'last_fetch': 0,
    'fetching': False,
    'error': None,
}

FETCH_INTERVAL = 10  # seconds


def _detect_backend():
    """Detect AdGuard or Pi-hole from environment."""
    ag_url = os.environ.get('ADGUARD_URL', 'http://192.168.1.50')
    ag_user = os.environ.get('ADGUARD_USER', 'admin')
    ag_pass = os.environ.get('ADGUARD_PASS', '')
    ph_url = os.environ.get('PIHOLE_URL', '')
    ph_key = os.environ.get('PIHOLE_API_KEY', '')

    if ph_url:
        return 'pihole', ph_url, ph_key, '', ''
    return 'adguard', ag_url, '', ag_user, ag_pass


def _do_fetch():
    """Background fetch worker."""
    backend, url, api_key, user, passwd = _detect_backend()
    try:
        if backend == 'pihole':
            data = _fetch_pihole(url, api_key)
        else:
            data = _fetch_adguard(url, user, passwd)
        _cache['data'] = data
        _cache['error'] = data.get('error')
    except Exception as e:
        _cache['error'] = str(e)[:80]
    finally:
        _cache['fetching'] = False
        _cache['last_fetch'] = time.time()


def _fetch():
    now = time.time()
    if now - _cache['last_fetch'] < FETCH_INTERVAL:
        return
    if _cache['fetching']:
        return
    _cache['fetching'] = True
    t = threading.Thread(target=_do_fetch, daemon=True)
    t.start()


def init():
    _cache['last_fetch'] = 0
    _cache['data'] = {}
    _cache['error'] = None
    _fetch()


# ── Sparkline helper ───────────────────────────────────────────────
def _draw_sparkline(draw, img, x, y, w, h, data, color):
    if not data or len(data) < 2:
        draw.rectangle([x, y, x + w, y + h], fill=(10, 13, 20))
        return
    draw.rectangle([x, y, x + w, y + h], fill=(10, 13, 20))
    mx = max(data) or 1
    points = []
    for i, val in enumerate(data):
        px = x + int(i * w / (len(data) - 1))
        py = y + h - int(min(val, mx) * h / mx)
        points.append((px, py))
    dim_fill = tuple(max(0, c - 160) for c in color)
    fill_points = points + [(x + w, y + h), (x, y + h)]
    draw.polygon(fill_points, fill=dim_fill)
    pad = 8
    gw, gh = w + pad * 2, h + pad * 2
    glow = Image.new('RGBA', (gw, gh), (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    shifted = [(px - x + pad, py - y + pad) for px, py in points]
    gd.line(shifted, fill=color + (140,), width=5)
    glow = glow.filter(ImageFilter.GaussianBlur(radius=6))
    img.paste(glow, (x - pad, y - pad), glow)
    draw.line(points, fill=tuple(min(255, c + 40) for c in color), width=2)


# ── Render ─────────────────────────────────────────────────────────
def _fmt_num(n):
    if n >= 1_000_000:
        return f"{n / 1_000_000:.1f}M"
    if n >= 1_000:
        return f"{n / 1_000:.1f}K"
    return str(int(n))


def render_frame(w=1920, h=440):
    _fetch()

    img = _get_bg(w, h)
    draw = ImageDraw.Draw(img)
    d = _cache['data']
    pad = 10

    # ── Setup/auth error state ──
    if not d or (_cache['error'] and not d.get('total_queries')):
        err = _cache['error'] or 'Connecting...'
        draw.text((pad + 20, 20), "DNS BLOCKER", fill=ACCENT, font=font(28))

        if '401' in str(err) or 'Unauthorized' in str(err):
            draw.text((pad + 20, 70), "Authentication required", fill=YELLOW, font=font(24))
            draw.text((pad + 20, 110), "Set environment variables:", fill=TEXT_DIM, font=font(20))
            draw.text((pad + 20, 145), "ADGUARD_URL=http://192.168.1.50", fill=CYAN, font=font(18))
            draw.text((pad + 20, 175), "ADGUARD_USER=admin", fill=CYAN, font=font(18))
            draw.text((pad + 20, 205), "ADGUARD_PASS=yourpassword", fill=CYAN, font=font(18))
            draw.text((pad + 20, 250), "Or for Pi-hole:", fill=TEXT_DIM, font=font(20))
            draw.text((pad + 20, 280), "PIHOLE_URL=http://your-pihole", fill=CYAN, font=font(18))
            draw.text((pad + 20, 310), "PIHOLE_API_KEY=your-api-key", fill=CYAN, font=font(18))
        elif _cache['fetching']:
            draw.text((w // 2 - 80, h // 2 - 12), "Connecting...", fill=ACCENT, font=font(24))
        else:
            draw.text((pad + 20, 70), f"Error: {err}", fill=RED, font=font(20))
            draw.text((pad + 20, 110), "Check ADGUARD_URL or PIHOLE_URL", fill=TEXT_DIM, font=font(18))

        img = Image.alpha_composite(img, _get_scanlines(w, h))
        out = Image.new('RGB', (w, h), BG)
        out.paste(img, (0, 0), img)
        return out

    # ── Main dashboard ──
    gap = 8
    content_y = pad
    content_h = h - pad * 2

    # Panel 1: Stats overview (left, 30%)
    p1w = int(w * 0.28)
    _draw_panel(draw, img, pad, content_y, p1w, content_h,
                accent_color=GREEN if d.get('protection') else RED)
    draw = ImageDraw.Draw(img)

    backend = d.get('backend', 'DNS')
    prot_status = "PROTECTED" if d.get('protection') else "DISABLED"
    prot_color = GREEN if d.get('protection') else RED
    draw.text((pad + 16, content_y + 8), f"{backend} — {prot_status}",
              fill=prot_color, font=font(20))

    # Total queries
    ty = content_y + 40
    draw.text((pad + 16, ty), "QUERIES", fill=TEXT_DIM, font=font(16))
    _draw_hero_text(draw, img, pad + 16, ty + 22, _fmt_num(d.get('total_queries', 0)),
                    ACCENT, 48)
    draw = ImageDraw.Draw(img)

    # Blocked
    ty += 90
    draw.text((pad + 16, ty), "BLOCKED", fill=TEXT_DIM, font=font(16))
    blocked_pct = d.get('blocked_pct', 0)
    _draw_hero_text(draw, img, pad + 16, ty + 22,
                    f"{_fmt_num(d.get('blocked', 0))}  ({blocked_pct:.1f}%)",
                    RED, 32)
    draw = ImageDraw.Draw(img)

    # Block rate bar
    ty += 70
    _draw_glow_bar(draw, img, pad + 16, ty, p1w - 32, 16, blocked_pct,
                   RED if blocked_pct > 20 else ORANGE if blocked_pct > 10 else GREEN)
    draw = ImageDraw.Draw(img)

    # Avg response time
    ty += 35
    avg_ms = d.get('avg_ms', 0)
    if avg_ms > 0:
        draw.text((pad + 16, ty), "AVG RESPONSE", fill=TEXT_DIM, font=font(16))
        ms_color = GREEN if avg_ms < 50 else YELLOW if avg_ms < 100 else RED
        draw.text((pad + 16, ty + 22), f"{avg_ms:.1f} ms", fill=ms_color, font=font(28))

    # Panel 2: Query timeline (center, 40%)
    p2x = pad + p1w + gap
    p2w = int(w * 0.40)
    _draw_panel(draw, img, p2x, content_y, p2w, content_h, accent_color=ACCENT)
    draw = ImageDraw.Draw(img)

    draw.text((p2x + 16, content_y + 8), "QUERIES OVER TIME", fill=ACCENT, font=font(18))

    # Queries sparkline
    queries_ot = d.get('queries_over_time', [])
    spark_y = content_y + 36
    spark_h = (content_h - 50) // 2 - 10
    _draw_sparkline(draw, img, p2x + 16, spark_y, p2w - 32, spark_h,
                    queries_ot[-48:] if queries_ot else [], ACCENT)
    draw = ImageDraw.Draw(img)
    draw.text((p2x + 16, spark_y + spark_h + 4), "Total queries", fill=TEXT_DIM, font=font(14))

    # Blocked sparkline
    blocked_ot = d.get('blocked_over_time', [])
    spark_y2 = spark_y + spark_h + 26
    _draw_sparkline(draw, img, p2x + 16, spark_y2, p2w - 32, spark_h,
                    blocked_ot[-48:] if blocked_ot else [], RED)
    draw = ImageDraw.Draw(img)
    draw.text((p2x + 16, spark_y2 + spark_h + 4), "Blocked queries", fill=TEXT_DIM, font=font(14))

    # Panel 3: Top blocked domains (right, 30%)
    p3x = p2x + p2w + gap
    p3w = w - p3x - pad
    _draw_panel(draw, img, p3x, content_y, p3w, content_h, accent_color=RED)
    draw = ImageDraw.Draw(img)

    draw.text((p3x + 16, content_y + 8), "TOP BLOCKED", fill=RED, font=font(18))

    top_blocked = d.get('top_blocked', [])
    ty = content_y + 34
    row_h = min(28, (content_h - 50) // max(len(top_blocked), 1))
    for i, item in enumerate(top_blocked[:12]):
        if isinstance(item, dict):
            domain = list(item.keys())[0]
            count = list(item.values())[0]
        else:
            domain = str(item)
            count = ''
        ry = ty + i * row_h
        if ry + row_h > h - pad:
            break
        # Truncate domain
        domain_short = domain[:30] if len(domain) > 30 else domain
        draw.text((p3x + 16, ry), domain_short, fill=TEXT, font=font(15))
        if count:
            draw.text((p3x + p3w - 70, ry), str(count), fill=TEXT_DIM, font=font(15))

    # ── Bottom accent ──
    for i in range(8):
        a = int(180 * (1.0 - i / 8))
        draw.line([(0, h - 1 - i), (w, h - 1 - i)], fill=ACCENT + (a,))

    # Scanlines + convert
    img = Image.alpha_composite(img, _get_scanlines(w, h))
    out = Image.new('RGB', (w, h), BG)
    out.paste(img, (0, 0), img)
    return out


if __name__ == '__main__':
    import sys
    if '--once' in sys.argv:
        init()
        time.sleep(3)
        img = render_frame()
        img.save('/tmp/pihole.png')
        print("Saved to /tmp/pihole.png")
    else:
        print("Usage: python3 pihole.py --once")
        print("Set ADGUARD_URL, ADGUARD_USER, ADGUARD_PASS env vars")
        print("Or PIHOLE_URL, PIHOLE_API_KEY for Pi-hole")
